    ...
    async function load() {
      ...
      try {
        ...
        if (coursRaw.length === 0) {
          try {
            const cRes = await fetch("/cours/index.json", {
              cache: "no-store",
            });
            if (cRes.ok) {
...
function HeroCarousel({ autoPlay = true, autoPlayInterval = 3000, role }) {
  ...
  const slides = [
    ...
    },
    {
      ...
      description:
        "Découvrez une vaste collection de cours organisés par matière et niveau. Accédez aux ressources pédagogiques de qualité pour enrichir vos connaissances.",
      buttons: [
    ...
    },
    {
      ...
      description:
        "Entraînez-vous avec nos exercices pratiques et TDs. Mettez en pratique vos connaissances théoriques avec des cas concrets et des corrections détaillées.",
      buttons: [
        {
          text: "Découvrir les TDs",
          link: "/td",
      ...
      <div className="carousel-container">
        ...
        >
          {slides.map((slide, index) => (
            <div
              key={slide.id}
              className={`carousel-slide slide-${slide.type}`}
            >
              {/* Background Image */}
              <div
